...
void run_flash_phase(BslProtocol& bsl, const upac::PacReader& reader, const fs::path& tmp_dir, const std::vector<std::string>& skip_list, const std::vector<std::string>& only_list, FlashMode flash_mode, bool fast_mode_active, bool debug_fast_lane_enabled) {
    ...
    for (const auto& fc : config.files) {
        ...
        auto execute_download = [&]() -> bool {
            ...
            public:
                PacketPipeline(BslProtocol& proto, const std::vector<uint8_t>& buffer, size_t chunk, bool enabled, size_t max_depth)
                    : proto_(proto), buffer_(buffer), chunk_size_(chunk), enabled_(enabled), max_depth_(max_depth) {}
                ...
                void reset(size_t offset, size_t chunk) {
                    stop();
                    {
                        std::lock_guard<std::mutex> lock(mutex_);
                        chunk_size_ = chunk;
            ...
            private:
                void run() {
                    while (true) {
                        ...
                        packet.frame = proto_.build_midst_data_packet(buffer_.data() + packet.offset, static_cast<uint32_t>(packet.size));
                        {
                            std::lock_guard<std::mutex> lock(mutex_);
            ...
            for (size_t offset = 0; offset < buf.size();) {
                ...
                if (!sent_ok) {
                    ...
                    if (chunk_size > min_chunk_size) {
                        size_t failed_chunk_size = chunk_size;
                        if (large_transfer_profile && remaining > 0x20000) {
